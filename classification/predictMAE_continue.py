...
EPOCHS = 10
MODEL_SAVE_PATH_PATTERN = "model_at_epoch_{epoch}_allrecording_3090_MAE_STFT_LOSS.h5"

def mse_stft_combined_loss(alpha=0.5):
	def loss(y_true, y_pred):
		# Convert y_true and y_pred to float32 for STFT
		y_true_float32 = tf.cast(y_true, tf.float32)
		y_pred_float32 = tf.cast(y_pred, tf.float32)
		
		# Mean Squared Error part
		mse_loss = tf.reduce_mean(tf.keras.losses.mean_squared_error(y_true_float32, y_pred_float32))
		
		# STFT part
		stft_true = tf.signal.stft(y_true_float32, frame_length=256, frame_step=128)
		stft_pred = tf.signal.stft(y_pred_float32, frame_length=256, frame_step=128)
...
# Function to find the last saved model
...
class TransformerBlock(tf.keras.layers.Layer):
	def __init__(self, embed_size, num_heads, forward_expansion, rate=0.1):
		...
		self.ffn = tf.keras.Sequential(
			[Dense(forward_expansion, activation="sigmoid"), Dense(embed_size),]
		)
...
# Check for the last saved model
model_dir = "./"
